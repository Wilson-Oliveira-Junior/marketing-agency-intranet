...
class NovoComentarioMail extends Notification
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        if(is_null($email_responsavel_da_tarefa)){
            $idEquipe = Tarefa::Where('id', $notifiable->id_tarefa)->value('id_equipe');
            $email_responsavel_da_tarefa = SetorUsuario::Where('id', $idEquipe)->value('email');
        }
        ...
        return (new MailMessage())->bcc($Arr)
                ->subject('[LD] NOVO Comentário na Tarefa #'.$notifiable->id_tarefa .' - ' . $titulo)
