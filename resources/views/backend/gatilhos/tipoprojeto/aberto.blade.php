...
@section('content')
    <div class="col">
        ...
        <div class="card shadow">
            ...
            <div class="card-header border-0 min-height-processo" style="display: flex;position: relative;">
                
                <!-- Título -->
                <h3 class="mb-0">Listagem de Gatilhos em Processo " - {{ $projeto->nome }} - "</h3> 

                <!-- Botao Voltar -->
                <a href="{{ route('backend.gatilhos.geral') }}" class="btn btn-outline-primary botao-processo">VOLTAR</a>

                <!-- Busca -->
                <div class="form-group busca-processo">
                    ...
                    <tbody>
                        @foreach($gatilhos as $registro)		
                           @if($registro->status_gatilho > 0) 		
                            <tr class="bloco">
                                <td>
                                    @if($registro->status_entrar_contato == 'N')
