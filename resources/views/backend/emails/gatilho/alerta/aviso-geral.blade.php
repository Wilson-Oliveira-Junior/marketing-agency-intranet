    ...
    <body>
        <div class="col-md-12 style-email">
            ...
            <div class="bloco-texto">
                <p>Olá Equipe,</p>
                <p>Viemos por meio desse e-mail lembra-los que as datas estão se aproximando.</p>
                <p>Bora Finalizar 🍻</p>
                ...
                <table>
                    ...
                    @foreach($arrProjetos as $projeto)
                        ...
                        @if($projeto['qtdeGatAviso'] > 0)

                                @if(count($projeto['gatilhosAviso'])> 0)
                                    @foreach ($projeto['gatilhosAviso'] as $key => $aviso)
                                    <tr>
                                        <td>{{$projeto['cliente']}}</td>
                                        <td>{{$projeto['projeto']}}</td>
                                        <td>{{$aviso['gatilho']}}</td>
                                        <td>{{date('d/m/Y', strtotime($aviso['data_limite']))}}</td>
                                        <td>{{$aviso['envolvidos']}}</td>
