    ...
    <body>
        <div class="col-md-12 style-email">
            ...
            <div class="bloco-texto">
                <p>Olá Pessoal,</p>
                <p>Andamento dos Projetos!<br>Total: {{count($gatilhos)}}</p>
                @foreach($gatilhos as $gatilho)
                    <p><span>{{$gatilho['nome']}}</span> - Último Contato: @if(DateTime::createFromFormat('Y-m-d H:i:s', $gatilho['ultimo_contato']) !== false) {{date('d/m/Y',strtotime($gatilho['ultimo_contato']))}} @endif / {{$gatilho['gatilhos'] - $gatilho['finalizados']}} gatilhos abertos </p>
                    <p class="mensagem"><span>Andamento: </span>{{date('d/m/Y',strtotime($gatilho['ultimo_contato']))}} - {{$gatilho['ultimo_comentario']}}<br/>Situação:
                    @if($gatilho['entraremcontato'])
                        <b style="color:red;font-size:12px">Entrar em contato</b>
                    @else
                        <b style="color:green;font-size:12px">Contato em dia.</b>
                    @endif
                    </p>
                @endforeach
            </div>
